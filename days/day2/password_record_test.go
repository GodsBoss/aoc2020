...
func TestPasswordRecordIsValid(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"first valid":  {"1-3 a: abcde", true, true},
		"invalid":      {"1-3 b: cdefg", false, false},
		"second valid": {"2-9 c: ccccccccc", true, false},
	}
	...
	for name, testcase := range testcases {
		t.Run(
			...
			func(t *testing.T) {
				...
				if actualIsValid != testcase.isValidPart1 {
					t.Errorf("expected isValid to be %t, but is %t", testcase.isValidPart1, actualIsValid)
				}
				actualIsReallyValid := pwRec.isReallyValid()
				if actualIsReallyValid != testcase.isValidPart2 {
					t.Errorf("expected isReallyValid to be %t, but is %t", testcase.isValidPart2, actualIsReallyValid)
				}
			},
		)
	}
